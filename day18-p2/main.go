...
func parseExp(tokens *[]token) expression {
	var lExp = parseMultiplies(tokens)

	for (*tokens)[0].tokenType.in(MUL){
		t := (*tokens)[0]
		*tokens = (*tokens)[1:]
		var exp expression
		switch t.tokenType {
...
func parseMultiplies(tokens *[]token) expression {
	var lExp = parseExpOperation(tokens)
	for (*tokens)[0].tokenType == ADD {
		t := (*tokens)[0]
		*tokens = (*tokens)[1:]
		var exp expression
		switch t.tokenType {
